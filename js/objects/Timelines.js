function Timeline() {
  ...
  this.createTimeline = function(){
    for (corpus in spriteManager.spriteGroups) {
      if (!this.timelineData.hasOwnProperty(corpus)){
        ...
        TimeKnots.draw(id, this.timelineData[corpus], {dateDimension:false, color: "#5c5c8a", width:this.timeline_width, height: '50', showLabels: true, labelFormat: "%Y",lineWidth:2});
      }
    }
  }
  ...
  this.resizeTimeline = function()
  {
    ...
    for (tm in this.timelineData) {
      ...
      var elem = $(id);
      elem.html('');
      var timeline = TimeKnots.draw(id, this.timelineData[tm], {dateDimension:false, color: "#5c5c8a", width:this.timeline_width, height: '50', showLabels: true, labelFormat: "%Y",lineWidth:2});
    }
  }
}
